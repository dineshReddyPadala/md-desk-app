...
async function exportList(req, reply) {
  ...
  return sendWorkbook(reply, 'projects_export.xlsx', [{
    ...
    rows: items.map((item) => ({
      ...
      TeamMembers: item.assignees?.map((assignee) => assignee.employee?.name).filter(Boolean).join(', ') || '',
      TeamEmails: item.assignees?.map((assignee) => assignee.employee?.email).filter(Boolean).join(', ') || '',
